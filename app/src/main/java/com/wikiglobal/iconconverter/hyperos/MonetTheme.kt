...
enum class MonetGlyphSource { NATIVE_MONOCHROME, SAFE_ADAPTIVE_FOREGROUND, GENERATED_MASK, SAFE_ICON_PACK_GLYPH, UNAVAILABLE_NO_SOURCE, UNAVAILABLE_EMPTY, UNAVAILABLE_FULL_BLEED, UNAVAILABLE_OPAQUE_EDGES, UNAVAILABLE_EXCESSIVE_COVERAGE }
...
object MonochromeResolver {
    fun resolve(icon:Drawable?, pack:Drawable?=null, c:GlyphSafetyConfig=GlyphSafetyConfig()):MonetGlyphResult { val mono=(icon as? AdaptiveIconDrawable)?.takeIf{Build.VERSION.SDK_INT>=33}?.monochrome; if(mono!=null)return native(mono,c); if(icon is AdaptiveIconDrawable){val fg=candidate(icon.foreground,MonetGlyphSource.SAFE_ADAPTIVE_FOREGROUND,c);if(fg.alphaMask!=null)return fg}; val extracted=(pack?:icon)?.let { GeneratedGlyphExtractor.extract(it, safety=c) }; if(extracted?.mask!=null)return MonetGlyphResult(extracted.mask,MonetGlyphSource.GENERATED_MASK,extracted.analysis); return pack?.let{candidate(it,MonetGlyphSource.SAFE_ICON_PACK_GLYPH,c)}?:MonetGlyphResult(null,extracted?.rejected?:MonetGlyphSource.UNAVAILABLE_NO_SOURCE,extracted?.analysis,extracted?.rejected?:MonetGlyphSource.UNAVAILABLE_NO_SOURCE) }
