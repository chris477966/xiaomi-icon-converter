...
class HyperIconApplication : Application(), DefaultLifecycleObserver {
    private val wallpaperReceiver=object:BroadcastReceiver(){override fun onReceive(context:android.content.Context,intent:Intent){if(Intent.ACTION_WALLPAPER_CHANGED==intent.action)WallpaperMonetScheduler.enqueue(this@HyperIconApplication)}}
